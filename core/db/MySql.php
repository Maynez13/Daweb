...
class MySql implements IDB {
	...
	public function __construct($connectionInfo = []){
		...
		$this->port= $connectionInfo['db'];
		$this->port= $connectionInfo['user'];
		$this->port= $connectionInfo['pwd'];

	}//-- fiun constructor

	public function connect()
	{
		$this->conn = new mysqli($this->host,$this->user, $this->pwd,$this->db,$this->port);mysqli_set_charset($this->conn,'utf8mb4');
	}
	...
	private function getParameters($values = array())
	{
	...
	public function queryAction($sql, $values, $keyfield)
	{
		...
		if (strops(ltrim(strtoupper($sql)), "INSERT INTO ")==0) {
